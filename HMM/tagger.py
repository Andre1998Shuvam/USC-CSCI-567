...
def model_training(train_data, tags):
    ...
    ###################################################
    word2idx = {list(unique_words.keys())[i]: i
                for i in range(len(unique_words))}
    tag2idx = dict({tags[i]: i for i in range(S)})
    ...
    #   to be zero.
    ###################################################
    state_transition_count = np.zeros((S))
    state_count = np.zeros((S))
    
    for line in train_data:
        pi[tag2idx[line.tags[0]]] += 1
        for i in range(1, len(line.tags)):
            tag_1 = tag2idx[line.tags[i-1]]
            tag_2 = tag2idx[line.tags[i]]
            word = word2idx[line.words[i-1]]
            A[tag_1][tag_2] += 1
            B[tag_1][word] += 1
            state_transition_count[tag_1] += 1
            state_count[tag_1] += 1
            
        B[tag2idx[line.tags[-1]]][word2idx[line.words[-1]]] += 1
        state_count[tag2idx[line.tags[-1]]] += 1    
    
    pi = pi / len(train_data)
    for s in range(S):
        A[s] = A[s] / state_transition_count[s] if state_transition_count[s] != 0 else 0
        B[s] = B[s] / state_count[s] if state_count[s] != 0 else 0
    # DO NOT MODIFY BELOW
...
def sentence_tagging(test_data, model, tags):
    ...
    for line in test_data:
        for i in line.words:
            if i not in model.obs_dict:
                model.obs_dict.update({i: len(model.obs_dict)})
                model.B = np.concatenate(
                    (model.B, np.zeros((len(tags), 1))), 1)
                model.B[:, -1] = 1e-6
        seq = model.viterbi(line.words)
        tagging.append(seq)
